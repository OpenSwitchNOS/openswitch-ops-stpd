...
{
   /*
    * This is an explicite case to ensure that we set the Port Roles
    * and state machines properly on the Standby. This will make sure
    * that we dont recompute the tree when we go from Standby to Active.
    * Disabling of a port will go thru several iterations to get to this
    * state. But on Standby we wont iterate thru all these trnasitions.
    * During Active Sync I was having issues not syncing this info onto
    * Standby. We dont need to do this during Bulk Sync.
    */
