...
mstp_removeLport(LPORT_t lport)
{
    ...
    *------------------------------------------------------------------------*/
#ifdef OPS_MSTP_TODO
   mstp_updateMstpCBPortMaps(&lports->port_map);
#endif /*OPS_MSTP_TODO*/

}

/**PROC+**********************************************************************
 * Name:      mstp_checkPvstHandlingConfig
 *
 * Purpose:   Check if any ports are set for any PVST handling.
 *
 * Params:    none
 *
 ...
 *
 * Constraints:
 *            MSTP_SEMID must be held already.
 * PROC-**********************************************************************/
bool
mstp_checkPvstHandlingConfig(void)
{
    bool answer = FALSE;

    bool bFilteredPorts = FALSE;
    bool bProtectedPorts = FALSE;

    bFilteredPorts = MSTP_COMM_ARE_ANY_PVST_FILTER_SET();

    bProtectedPorts = MSTP_COMM_ARE_ANY_PVST_PROTECT_SET();

    if (bFilteredPorts || bProtectedPorts) {
        answer = TRUE;
    }
...
/**PROC+**********************************************************************
 * Name:      mstp_enablePvstHandling
 *
 * Purpose:   Program the hardware with the PVST multicast
 *            address.
 *
 * Params:    none
 *
 * Returns:   none
 *
 * Globals:   pvst_multicast
 *
 * Constraints:
 *            MSTP_SEMID must be held already.
 * PROC-**********************************************************************/
void
mstp_enablePvstHandling(void)
{
   if(!mstp_Bridge.pvstmac_enable)
   {
      mstp_Bridge.pvstmac_enable = TRUE;
   }
}
...
mstp_updateMstiVidMapping(MSTID_t mstid, VID_MAP newVidMap)
{
   ...
   {/* VID mapping has changed for this MST Instance */
      ...
      if(are_any_vids_set(&delVidMap))
      {
         ...
         /*------------------------------------------------------------------
          * From the whole list of VLAN IDs find the ones that are physically
          * present on the switch and update DB to remove them from the
          * VLAN group associated with given MSTI.
          *------------------------------------------------------------------*/
         for(vid = find_first_vid_set(&delVidMap);
             IS_VALID_VID(vid);
             vid = find_next_vid(&delVidMap, vid))
         {
            /* Obtain the internal VLAN number corresponding to the VID
             * NOTE: 'get_vlan_number_for_vid' returns 1..MAX_VLAN number
             *       on success and '0' otherwise */
#if OPS_MSTP_TODO
             send_msti_vlan_update_msg();
#endif /*OPS_MSTP_TODO*/
         }
      ...
      if(are_any_vids_set(&addVidMap))
      {
         ...
         /*------------------------------------------------------------------
          * From the whole list of VLAN IDs find the ones that are physically
          * present on the switch and request IDL to add them to the
          * VLAN group associated with given MSTI.
          *------------------------------------------------------------------*/
         for(vid = find_first_vid_set(&addVidMap);
             IS_VALID_VID(vid);
             vid = find_next_vid(&addVidMap, vid))
         {
            /* Obtain the internal VLAN number corresponding to the VID
             * NOTE: 'get_vlan_number_for_vid' returns 1..MAX_VLAN number
             *       on success and '0' otherwise */
