...
mstp_util_add_default_ports_to_cist() {
    ...
    cist_port_info =
    xcalloc(bridge_row->n_ports, sizeof *cist_row->mstp_common_instance_ports);
    if(!cist_port_info) {
...
mstp_show_common_instance_info(
    ...
    system_row = ovsrec_system_first(idl);
    if (!system_row) {
        vty_out(vty, "no system record found%s", VTY_NEWLINE);
        return e_vtysh_error;
    }

    /* common instance table details */
    vty_out(vty, "%-14s %s%-14s", "#### MST0", VTY_NEWLINE, "vlans mapped:");
    if (cist_row->vlans) {
    ...
    vty_out(vty, "%-14s %s:%ld  %s:%ld  %s:%ld  %s:%ld%s", "Operational",
            ...
            VTY_NEWLINE);
    vty_out(vty, "%-14s %s:%ld  %s:%ld %s:%ld  %s:%ld%s", "Configured",
            "Hello time(in seconds)",
...
mstp_show_instance_info(const struct ovsrec_mstp_common_instance *cist_row,
    ...
    for (i=0; i < bridge_row->n_mstp_instances; i++) {
        ...
        vty_out(vty, "%s%s%ld %s%-14s", VTY_NEWLINE, "#### MST",
...
cli_show_mstp_global_config() {
    ...
    if (bridge_row) {
        if (bridge_row->mstp_enable &&
                (*bridge_row->mstp_enable != DEF_ADMIN_STATUS)) {
            vty_out(vty, "spanning-tree enable.%s", VTY_NEWLINE);
        }
...
/*-----------------------------------------------------------------------------
 | Function:        cli_show_mst
 | Responsibility:  Displays running-config for MSTP module(interface-level)
 | Parameters:
 | Return:
 |      Return : e_vtysh_ok on success else e_vtysh_error
 ------------------------------------------------------------------------------
...
mstp_cli_set_mst_inst(const char *if_name,const char *key,
    ...
    if (!mstp_row) {
        ERRONEOUS_DB_TXN(txn,
                "No MSTP instance found with with This instance ID");
    }
...
/*-----------------------------------------------------------------------------
 ...
 |     vlanid:     VLAN ID
 |     operation:  MSTP_ADD_VLAN_TO_INSTANCE - Add new vlan to existing instance
 |                 MSTP_REMOVE_VLAN_FROM_INSTANCE - Remove vlan from instance
 ...
 */
static int
mstp_cli_inst_vlan_map(const int64_t instid, const char *vlanid,
        int64_t operation) {
    ...
    for (i=0; i < bridge_row->n_mstp_instances; i++) {
        ...
        if ((bridge_row->key_mstp_instances[i] == instid)&&
                (operation == MSTP_ADD_VLAN_TO_INSTANCE)) {
        ...
        /* Removing a VLAN from existing instance */
                ...
                (operation == MSTP_REMOVE_VLAN_FROM_INSTANCE)) {
        ...
        /* Removing a complete MSTP instance */
                ...
                (operation == MSTP_REMOVE_INSTANCE)) {

            instId_list =
                xcalloc(bridge_row->n_mstp_instances -1, sizeof(int64_t));
            if (!instId_list) {
                ERRONEOUS_DB_TXN(txn, "Memory allocation failed");
            }
...
DEFUN(cli_mstp_inst_vlanid,
    ...
    mstp_cli_inst_vlan_map (atoi(argv[0]), argv[1], MSTP_ADD_VLAN_TO_INSTANCE);
    return CMD_SUCCESS;
}
...
DEFUN(cli_no_mstp_inst_vlanid,
      ...
      VLAN_STR
      "VLAN to add or to remove from the MST instance\n") {
    mstp_cli_inst_vlan_map (atoi(argv[0]), argv[1], MSTP_REMOVE_VLAN_FROM_INSTANCE);
    return CMD_SUCCESS;
}
...
DEFUN(cli_no_mstp_inst,
      ...
      MST_INST
      "Enter an integer number\n") {
    mstp_cli_inst_vlan_map (atoi(argv[0]), NULL, MSTP_REMOVE_INSTANCE);
    return CMD_SUCCESS;
}
