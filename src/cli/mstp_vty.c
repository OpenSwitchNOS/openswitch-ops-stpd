...
cli_show_spanning_tree_config(bool detail) {
    ...
    if (VTYSH_STR_EQ(system_row->system_mac, root_mac)) {
        ...
        /* if CIST root is other ext path cost exist print the cost else get the internal path cost */
        vty_out(vty, "%17s%8s %ld%s", "Cost",":",
        (cist_row->cist_path_cost)?(*cist_row->cist_path_cost):((cist_row->root_path_cost)?(*cist_row->root_path_cost):0),
        VTY_NEWLINE);
        vty_out(vty, "%17s%8s %s%s", "Port",":",
                (cist_row->root_port)?cist_row->root_port:"0", VTY_NEWLINE);
    }
