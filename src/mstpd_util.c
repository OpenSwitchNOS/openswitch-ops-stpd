 ...
 **PROC-**********************************************************************/
bool
mstp_isLportFwdOnVlan(LPORT_t lport, VLAN_t vlan)
{
   bool res = FALSE;

   STP_ASSERT(IS_VALID_LPORT(lport));
   STP_ASSERT(IS_VALID_VLAN(vlan));
...
mstp_portDisable(LPORT_t lport)
{
   ...
   if(MSTP_COMM_PORT_IS_BIT_SET(commPortPtr->bitMap, MSTP_PORT_PORT_ENABLED))
   {
      ...
      /*---------------------------------------------------------------------
       * If we are not Active MM then we should not deal with the FSMs, as
       * the decision about the ports will be taken by the Active MM and will
       * pass that information to us.
       * if (IS_SW_SS_STATE_ACTIVE(mstp_CB.subSystemState))
       * Currently we are allowing the Standby to act on PORT_DOWN messages
       * since it will reset the FSMs.
       * TODO: Verify whether this is OK or should I have separate function
       * to set the FSM states to be dafult values???
       *---------------------------------------------------------------------*/
      /*---------------------------------------------------------------------
       * kick CIST's Port Information state machine (per-Tree per-Port)
       *---------------------------------------------------------------------*/
...
mstp_enableForwarding(MSTID_t mstid, LPORT_t lport)
{
   ...
   mstp_updtMstiPortNeedSync(mstid, lport);
   if(mstid == MSTP_CISTID)
   {
      STP_ASSERT(MSTP_CIST_PORT_PTR(lport));
...
mstp_updtRolesCist(void)
{
   ...
   for(lport = 1; lport <= MAX_LPORTS; lport++)
   {
      ...
      if(commPortPtr && cistPortPtr)
      {
          ...
          * Currently it is used only for Standby. In future can be used for
          * Distributed STP.
          *------------------------------------------------------------------*/
         if (selectedRole != MSTP_PORT_ROLE_UNKNOWN)
         {
            if ((selectedRole != cistPortPtr->selectedRole) || rootTimeChange)
            {
               mstp_updtMstiPortNeedSync(MSTP_CISTID, lport);
            }
...
mstp_updtRolesMsti(MSTID_t mstid)
{
   ...
   for(lport = 1; lport <= MAX_LPORTS; lport++)
   {
      ...
      if(commPortPtr && mstiPortPtr)
      {
         ...
         if (selectedRole != MSTP_PORT_ROLE_UNKNOWN)
         {
            if ((selectedRole != mstiPortPtr->selectedRole) || rootTimeChange)
            {
               mstp_updtMstiPortNeedSync(mstid, lport);
            }
...
/**PROC+**********************************************************************
 * Name:      mstp_updtMstiPortNeedSync
 *
 ...
 **PROC-**********************************************************************/
void
mstp_updtMstiPortNeedSync(MSTID_t mstid, LPORT_t lport)
{
   ...
   set_port(&m->portsNeedSync, lport);
}
...
#define MSTP_MAX_VID_STR_LEN 10 /* length of "xxxx-xxxx" + '\0' */
   ...
   tmp = vidStr;
   for(vid = MIN_VID; vid <= MAX_VLAN_ID + 1; vid++)
   {
...
/**PROC+**********************************************************************
 * Name:      mstp_portPvstFilterCheck
 *
 * Purpose:   returns TRUE, for PVST validation if the lport passing the bpdu
 *            is on the 'hpSwitchStpPortPvstFilter' data structure
 *
 * Params:    lport -> logical port number
 *
 * Returns:   TRUE if lport is part of the hpSwitchStpPortPvstFilter
 *
 * Globals:   mstp_Bridge, mstp_CB
 *
 * Constraints:
 **PROC-**********************************************************************/
bool
mstp_portPvstFilterCheck(LPORT_t lport)
{
   bool result;
   STP_ASSERT(IS_VALID_LPORT(lport));


   if(MSTP_COMM_IS_PVST_FILTER(lport))
   {/* drop the BPDU by not validating it */
      result = TRUE;
   }
   else
...
/**PROC+**********************************************************************
 * Name:      mstp_countPvstFilters
 *
 * Purpose:   Provides a public function that accesses and returns the number
 *            of PVST filters currently in place.
 *
 * Params:    none
 *
 * Returns:   Count of ports with PVST filtering enabled.
 *
 * Globals:   mstp_Bridge, mstp_CB
 *
 * Constraints:
 **PROC-**********************************************************************/
int
mstp_countPvstFilters(void)
{
   LPORT_t lport;
   int     count = 0;

   for (lport = (LPORT_t)find_first_port_set(&mstp_Bridge.pvstFilterLports);
        IS_VALID_LPORT(lport);
        lport = (LPORT_t)find_next_port_set(&mstp_Bridge.pvstFilterLports, lport)) {
