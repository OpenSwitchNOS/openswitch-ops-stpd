...
extern
VLAN_t find_first_vlan_set(const VLAN_MAP *map)
{
   if(map)
   {
      return((VLAN_t)findFirstBitSet(&map->vmap[0], MAX_VLANS));
   }
   else
   {
       assert(0);
...
extern
VLAN_t find_next_vlan(const VLAN_MAP *map, uint32_t prevVlan)
{
   if(map)
   {
      return((VLAN_t)findNextBitSet(&map->vmap[0], prevVlan, MAX_VLANS));
   }
   else
   {
       assert(0);
