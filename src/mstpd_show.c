...
/**PROC+**********************************************************************
 * Name:      mstpd_daemon_msg_queue_data_dump
 *
 * Purpose:   Display info about queued msgs to be send to DB
 *
 * Params:    none
 *
 * Returns:   none
 ...
 **PROC-**********************************************************************/
void
mstpd_daemon_msg_queue_data_dump(struct ds *ds, int argc, const char *argv[])
{
   MSTP_TREE_MSG_t *m;
   ...
   while(m_next != (MSTP_TREE_MSG_t*) Q_NULL)
   {
      ...
      fwd  = are_any_ports_set(&m->portsFwd);
      lrn  = are_any_ports_set(&m->portsLrn);
      blk  = are_any_ports_set(&m->portsBlk);
      up   = are_any_ports_set(&m->portsUp);
      dwn  = are_any_ports_set(&m->portsDwn);
      flsh = are_any_ports_set(&m->portsMacAddrFlush);
      ds_put_format(ds,"MSTI%d-> ", m->mstid);
      ds_put_format(ds,"fwd=%c;lrn=%c;blk=%c;up=%c;dwn=%c;flsh=%c\n",
             fwd  ? 'Y' : 'N',
             lrn  ? 'Y' : 'N',
             blk  ? 'Y' : 'N',
             up   ? 'Y' : 'N',
...
void mstpd_daemon_msg_queue_unixctl_list(struct unixctl_conn *conn, int argc,
                   const char *argv[], void *aux OVS_UNUSED)
{
    struct ds ds = DS_EMPTY_INITIALIZER;

    mstpd_daemon_msg_queue_data_dump(&ds, argc, argv);
    unixctl_command_reply(conn, ds_cstr(&ds));
    ds_destroy(&ds);
}
