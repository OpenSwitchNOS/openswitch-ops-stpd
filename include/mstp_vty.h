...
#define START_DB_TXN(txn)                                       \
    do {                                                        \
        ...
        if (txn == NULL) {                                      \
            VLOG_ERR("ovsdb_idl_txn_create failed: %s: %d\n",   \
                    __FILE__, __LINE__);                            \
...
#define ERRONEOUS_DB_TXN(txn, error_message)                        \
    do {                                                            \
        cli_do_config_abort(txn);                                   \
        VLOG_ERR("database transaction failed: %s: %d -- %s\n",     \
                __FILE__, __LINE__, error_message);                 \
        vty_out(vty, "%s", error_message);                          \
        return CMD_WARNING;                                         \
    } while (0)
...
#define ABORT_DB_TXN(txn, message)                             \
    do {                                                       \
        cli_do_config_abort(txn);                                   \
        VLOG_ERR("database transaction aborted: %s: %d, %s\n",  \
