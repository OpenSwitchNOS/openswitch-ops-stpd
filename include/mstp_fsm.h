...
typedef uint16_t VLAN_t;         /* Vlan  (1 to MAX_VLANS,  0 = NO_VLAN)     */
...
#define IS_VALID_VLAN(vlan) ((vlan >= 1) && (vlan <= MAX_VLANS))


#define IS_VALID_VID(vid)          ((vid >= MIN_VID) && (vid <= MAX_VID))
...
#define PRINT_MAC_ADDR_FASTLOG(a)       \
        *((uint16_t *)(a)),   *((uint16_t *)(a)+1), *((uint16_t *)(a)+2)
...
/*---------------------------------------------------------------------------
 *  * Path Cost value used to assign for MESH port (lowest possible value to
 *   * eliminate MESH port from being blocked by MSTP).
 *    *---------------------------------------------------------------------------*/
#define MSTP_PATH_COST_MESH_LPORT  1
#define MSTP_PATH_COST_DIST_TRK_LPORT 1
...
extern void     mstp_dynCreateVlan(VLAN_t vlan);
extern void     mstp_dynDeleteVlan(VLAN_t vlan);
...
extern bool     mstp_isLportFwdOnVlan(LPORT_t lport, VLAN_t vlan);
...
extern int      mstp_countPvstFilters(void);
...
#define MSTP_MESH_RECONFIG_CHANGE      (mstp_Bridge.meshDynReconfig)
#define MSTP_DIST_TRUNK_RECONFIG_CHANGE (mstp_Bridge.distTrunkReconfig)
...
/*---------------------------------------------------------------------------
 * Used to test, set and clear per-port PVST BPDU filter option
 *---------------------------------------------------------------------------*/
#define MSTP_COMM_IS_PVST_FILTER(p)    \
                           (is_port_set(&mstp_Bridge.pvstFilterLports,(p)))
#define MSTP_COMM_SET_PVST_FILTER(p)   \
                           (set_port(&mstp_Bridge.pvstFilterLports,(p)))
#define MSTP_COMM_CLR_PVST_FILTER(p)   \
                           (clear_port(&mstp_Bridge.pvstFilterLports,(p)))
#define MSTP_COMM_ARE_ANY_PVST_FILTER_SET()    \
                           (are_any_ports_set(&mstp_Bridge.pvstFilterLports))

/*
 *  Test, set, and clear (default) for per-port PVST BPDU Protection
 */
#define MSTP_COMM_PORT_IS_PVST_PROTECTED(p) \
                           (is_port_set(&mstp_Bridge.pvstProtectionLports,(p)))
#define MSTP_COMM_PORT_SET_PVST_PROTECTION(p) \
                           (set_port(&mstp_Bridge.pvstProtectionLports,(p)))
#define MSTP_COMM_PORT_CLR_PVST_PROTECTION(p) \
                           (clear_port(&mstp_Bridge.pvstProtectionLports,(p)))
#define MSTP_COMM_ARE_ANY_PVST_PROTECT_SET()    \
                           (are_any_ports_set(&mstp_Bridge.pvstProtectionLports))
...
typedef enum {
  ...
  MSTP_ERRANT_PVST_DATA_PKT,
  MSTP_UNAUTHORIZED_PVST_DATA_PKT,
  MSTP_PVST_TO_FORWARD_PKT,
  MSTP_INVALID_PKT
} MSTP_PKT_TYPE_t;
...
typedef struct MSTP_BRIDGE_INFO_t
{
   ...
   bool                        dynReconfig;     /* indicates whether dynamic
                                                  ...
                                                  * of MSTP                  */
   bool                        meshDynReconfig; /* indicates that non-MESH
                                                  * Port's internal path cost
                                                  * value has been modified
                                                  * to unblock the MESH port
                                                  * and MSTP needs to
                                                  * re-converge              */
   bool                        distTrunkReconfig;/* indicates that non-ISC
                                                  * Port's internal path cost
                                                  * value has been modified
                                                  * to unblock the ISC port
                                                  * and MSTP needs to
                                                  * re-converge              */
   ...
   uint16_t                     portReEnableTimeout;/* time (in seconds)
                                                  ...
                                                  * BPDUs                    */
   PORT_MAP                     pvstFilterLports;/* PVST BPDU-Filtered ports */
   /* Ports with pvst protection enabled */
   PORT_MAP                     pvstProtectionLports;/* PVST BPDU-Protected
                                                  * ports                    */
  bool                         pvstmac_enable; /* flag indicates if mac is
                                                   programed in */
...
extern void mstp_clearMstpToOthersMessageQueue(void);
extern void mstp_updateMstpCBPortMaps(PORT_MAP *pmap);
/*
 * mstp_util.c
 */
...
extern MSTID_t
            mstp_getMstIdForVlan(VLAN_t vlan);
...
extern void mstp_updtMstiPortNeedSync(MSTID_t mstid, LPORT_t lport);
...
extern bool mstp_mapVlanToMsti(MSTID_t msti, VLAN_t vlan);
...
extern bool isLportForwardingOnVlan(LPORT_t lport, VLAN_t vlan);
