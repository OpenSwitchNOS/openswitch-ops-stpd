...
def test_cist_single_region_root_elect(topology):
    ...
    for ops_show in [ops1_show, ops2_show, ops3_show]:
        ...
        if ops_show['root'] == 'yes':
            root_show = ops_show
            assert(ops_show[interface1]['role'] == 'Designated'), \
                "Port role has not updated correctly"

            assert(ops_show[interface2]['role'] == 'Designated'), \
                "Port role has not updated correctly"

            assert(ops_show[interface1]['State'] == 'Forwarding'), \
                "Port state has not updated correctly"

            assert(ops_show[interface2]['State'] == 'Forwarding'), \
                "Port state has not updated correctly"
        else:
    ...
    result = root_sw.send_command('ovs-vsctl list system | grep system_mac',
                                  shell='bash')
    result = re.search('\s*system_mac\s*:\s*"(?P<sys_mac>.*)"', result)
    result = result.groupdict()
    root = result['sys_mac']
