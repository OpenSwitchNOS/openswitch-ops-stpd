                                          ...
                                          struct mstp_instance_port *mstp_port)
{
    ...
    VLOG_DBG("%s: stg %d port name %s state %d", __FUNCTION__, msti->hw_stg_id,
             mstp_port->name, mstp_port->stp_state);

    ofproto_set_stg_port_state(br->ofproto, mstp_port->name, msti->hw_stg_id,
...
-----------------------------------------------------------------------------*/
void
mstp_instance_update(struct blk_params *br_blk_params,
                          struct mstp_instance *msti)
{
    ...
    if (OVSREC_IDL_IS_ROW_MODIFIED(p_mist_row,
    ...
    HMAP_FOR_EACH (inst_port, hmap_node, &msti->ports) {
        ...
        if (OVSREC_IDL_IS_ROW_MODIFIED(inst_port_row,
            ovsdb_idl_get_seqno(br_blk_params->idl))) {
...
-----------------------------------------------------------------------------*/
void
mstp_cist_configure_ports(const struct blk_params *br,
                             struct mstp_instance *msti)
{
    size_t i;
    struct mstp_instance_port *inst_port, *next;
...
-----------------------------------------------------------------------------*/
void
mstp_cist_update(const struct blk_params *br)
{
    struct mstp_instance *msti;
    const struct ovsrec_mstp_common_instance *msti_cist_cfg;
...
-----------------------------------------------------------------------------*/
void
mstp_update_instances(struct blk_params* br_blk_param)
{
       struct mstp_instance *msti, *next_msti;
...
stp_reconfigure(struct blk_params* br_blk_param)
{
    ...
    mstp_cist_update(br_blk_param);
    mstp_add_del_instances(br_blk_param);
    mstp_update_instances(br_blk_param);
}
